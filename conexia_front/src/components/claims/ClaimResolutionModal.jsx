...
export const ClaimResolutionModal = ({
  isOpen,
  onClose,
  claim,
  onSuccess,
  ...
  const normalized = useMemo(() => {
    ...
    const description =
      claimData?.claim?.description || claimObj?.description || "";
    const evidenceUrls =
      claimData?.claim?.evidenceUrls || claimObj?.evidenceUrls || [];
    ...
    const baseTypeLabel =
      claimObj?.claimTypeLabel || getClaimTypeLabel(claimType) || claimType;

    // Verificar si es tipo "Otro" (client_other o provider_other)
    const isOtherType =
      claimType === "client_other" || claimType === "provider_other";
    const claimTypeLabel =
      isOtherType && otherReason
    ...
    const claimantUserId =
      claimData?.claimant?.profile?.id ||
      claimData?.claimant?.id ||
      claimData?.claimant?.userId;
    const otherUserId =
      claimData?.otherUser?.profile?.id ||
      claimData?.otherUser?.id ||
      claimData?.otherUser?.userId;
  ...
  const updateCompliance = (id, field, value) => {
    setCompliances(
      compliances.map((c) => (c.id === id ? { ...c, [field]: value } : c)),
    );
  };
  ...
  const confirmCompliance = (id) => {
    ...
    setCompliances(
      compliances.map((c) => (c.id === id ? { ...c, isConfirmed: true } : c)),
    );
    setEditingComplianceId(null);
    setError(null);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (trimmedResolution.length < CLAIM_VALIDATION.RESOLUTION_MIN_LENGTH) {
      setError(
        `La resolución debe tener al menos ${CLAIM_VALIDATION.RESOLUTION_MIN_LENGTH} caracteres`,
      );
      return;
    }
    ...
    if (trimmedResolution.length > CLAIM_VALIDATION.RESOLUTION_MAX_LENGTH) {
      setError(
        `La resolución no puede exceder ${CLAIM_VALIDATION.RESOLUTION_MAX_LENGTH} caracteres`,
      );
      return;
    }
    ...
    if (
      ...
      partialAgreementDetails.trim().length >
        CLAIM_VALIDATION.PARTIAL_AGREEMENT_MAX_LENGTH
    ) {
      setError(
        `Los detalles del acuerdo no pueden exceder ${CLAIM_VALIDATION.PARTIAL_AGREEMENT_MAX_LENGTH} caracteres`,
      );
      return;
    }
    ...
    if (editingComplianceId !== null) {
      setError(
        "Debes confirmar o cancelar el compromiso actual antes de continuar",
      );
      return;
    }
    ...
    for (const compliance of confirmedCompliances) {
      ...
      if (compliance.instructions.trim().length < 20) {
        setError(
          "Las instrucciones de cada compromiso deben tener al menos 20 caracteres",
        );
        return;
      }
      if (compliance.instructions.trim().length > 1000) {
        setError(
          "Las instrucciones de cada compromiso no pueden exceder 1000 caracteres",
        );
        return;
      }
      if (compliance.deadlineDays < 1 || compliance.deadlineDays > 60) {
    ...
    try {
      ...
      if (confirmedCompliances.length > 5) {
        setError(
          `Error: Se detectaron ${confirmedCompliances.length} compromisos confirmados. El máximo es 5. Por favor, recarga la página.`,
        );
      ...
      if (confirmedCompliances.length > 0) {
        ...
        const uniqueCompliances = confirmedCompliances.filter(
          (compliance, index, self) =>
      ...
      if (showToast) {
        showToast(
          "error",
          err.message ||
            "Error al procesar la resolución. Por favor, intenta nuevamente.",
        );
      }
    } finally {
      setIsSubmitting(false);
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50">
              ...
              <div className="bg-white p-6 rounded-lg shadow-sm border">
                <h3 className="text-xl font-semibold text-conexia-green-dark mb-4">
                ...
                <div className="border-t pt-6 mt-6">
                  <label
                    htmlFor="resolution"
                    className="block text-sm font-semibold text-conexia-green-dark mb-2"
                  >
              ...
              <div className="bg-white p-6 rounded-lg shadow-sm border">
                <div className="mb-4">
                  <h3 className="text-xl font-semibold text-conexia-green-dark">
                    Compromisos a asignar
                  </h3>
                  <p className="text-sm text-gray-600 mt-1">
                    Define las acciones que cada parte debe cumplir para
                    resolver el reclamo
                  </p>
                </div>
                ...
                {compliances.filter((c) => c.isConfirmed).length > 0 && (
                  <div className="space-y-2 mb-4">
                    <p className="text-sm font-semibold text-gray-700">
                      ...
                      {compliances.filter((c) => c.isConfirmed).length}):
                    </p>
                    {compliances
                          ...
                          >
                            ...
                            <div className="absolute top-2 right-2 flex items-center gap-1">
                              <button
                                ...
                                disabled={
                                  isSubmitting || editingComplianceId !== null
                                }
                ...
                {editingComplianceId !== null &&
                  (() => {
                    const editingCompliance = compliances.find(
                      (c) => c.id === editingComplianceId,
                    );
                      ...
                      <div className="bg-purple-50 border-2 border-purple-300 rounded-lg p-4 mb-3">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <SelectField
                                ...
                                options={Object.entries(COMPLIANCE_TYPES).map(
                                  ([key, value]) => ({
                                    value: value,
                        ...
                        <div className="flex justify-end gap-2 mt-3 pt-3 border-t">
                          ...
                          <Button
                            type="button"
                            onClick={() =>
                              confirmCompliance(editingComplianceId)
                            }
                            variant="success"
                ...
                {compliances.length === 0 && (
                  <div className="text-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300 mt-4">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Los compromisos son opcionales. Puedes agregar hasta 5
                      compromisos
                    </p>
                  </div>
                )}
