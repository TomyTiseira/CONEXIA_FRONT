...
export default function MyClaimsPage() {
  ...
  const { claims, pagination, loading, error, updateFilters, setPage } =
    useMyClaims({
  ...
  const [showSelectComplianceModal, setShowSelectComplianceModal] =
    useState(false);
  const [showUploadEvidenceModal, setShowUploadEvidenceModal] = useState(false);
  const [showSelectForReviewModal, setShowSelectForReviewModal] =
    useState(false);
  ...
  const getShortDisplayName = (profile) => {
    ...
    const lastName = profile.lastName
      ? String(profile.lastName).trim().split(/\s+/)[0]
      : "";
  ...
  const handleComplianceAction = async (claim, actionType) => {
    try {
      ...
      const claimant =
        userRole === "claimant" ? result.yourProfile : result.otherUserProfile;
      const otherUser =
        userRole === "claimant" ? result.otherUserProfile : result.yourProfile;
        ...
        {error && !loading && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-8 text-center">
            ...
            <h3 className="text-xl font-semibold text-red-900 mb-3">
              No pudimos obtener tus reclamos
            </h3>
                          ...
                          >
                                ...
                                <img
                                  src={
                                    claim.otherUser?.profilePicture
                                      ? claim.otherUser.profilePicture.startsWith(
                                          "http",
                                        )
                                        ? claim.otherUser.profilePicture
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              {(() => {
                                ...
                                const pending = claim.compliances.filter(
                                  (c) => c.status === "pending",
                                ).length;
                                const approved = claim.compliances.filter(
                                  (c) => c.status === "approved",
                                ).length;
                                const submitted = claim.compliances.filter(
                                  (c) => c.status === "submitted",
                                ).length;
                                const isResponsible = claim.compliances.some(
                                  (c) =>
                                    String(c.responsibleUserId) ===
                                    String(user?.id),
                                );
                                ...
                                return (
                                  <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800 border border-gray-200">
                                    {total}{" "}
                                    {total === 1 ? "compromiso" : "compromisos"}
                                  </span>
                                );
                              })()}
                            </td>
        ...
        <UploadComplianceEvidenceModal
          ...
          onSubmit={async (evidenceData) => {
            try {
              ...
              const { submitComplianceEvidence } =
                await import("@/service/claims");
