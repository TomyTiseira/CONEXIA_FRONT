...
export default function EditPlanModal({ plan, onClose, onSave, loading }) {
  ...
  // Etiqueta legible para enums en selects
  const labelify = (val) => {
  ...
  const validateField = (field, value) => {
    ...
    switch (field) {
      case 'name':
        if (!value || value.trim() === '') newErrors.name = 'El nombre del plan es obligatorio';
        else if (value.length > 50) newErrors.name = 'El nombre no puede exceder 50 caracteres';
        else delete newErrors.name;
        break;
      case 'description':
        if (value && value.length > 200) newErrors.description = 'La descripción no puede exceder 200 caracteres';
        else delete newErrors.description;
        break;
      case 'monthlyPrice':
        if (!value || value === '') newErrors.monthlyPrice = 'El precio mensual es obligatorio';
        else if (isNaN(value) || parseFloat(value) < 0) newErrors.monthlyPrice = 'Debe ser un número válido mayor o igual a 0';
        else delete newErrors.monthlyPrice;
        break;
      case 'annualPrice':
        if (!value || value === '') newErrors.annualPrice = 'El precio anual es obligatorio';
        else if (isNaN(value) || parseFloat(value) < 0) newErrors.annualPrice = 'Debe ser un número válido mayor o igual a 0';
        else delete newErrors.annualPrice;
        break;
    }
    setErrors(newErrors);
  };
  ...
  const handleBenefitChange = (benefitKey, value, type) => {
    ...
    if (type === 'number') parsedValue = value === '' ? 0 : parseInt(value, 10);
    else if (type === 'boolean') parsedValue = value;
    ...
    setErrors((prev) => {
      const next = { ...prev };
      if (updatedBenefits.length === 0) next.benefits = 'Debes seleccionar al menos un beneficio';
  ...
  const isBenefitChecked = (benefit) => {
  ...
  const toggleBenefit = (benefit, checked) => {
    ...
    if (benefit.type === 'boolean') handleBenefitChange(benefit.key, true, 'boolean');
    else if (benefit.type === 'number') handleBenefitChange(benefit.key, 1, 'number');
  ...
  const validate = () => {
    const newErrors = {};
    if (!formData.name || formData.name.trim() === '') newErrors.name = 'El nombre del plan es obligatorio';
    else if (formData.name.length > 50) newErrors.name = 'El nombre no puede exceder 50 caracteres';

    if (formData.description && formData.description.length > 200)
      newErrors.description = 'La descripción no puede exceder 200 caracteres';

    if (!formData.monthlyPrice || formData.monthlyPrice === '') newErrors.monthlyPrice = 'El precio mensual es obligatorio';
    ...
    if (!formData.annualPrice || formData.annualPrice === '') newErrors.annualPrice = 'El precio anual es obligatorio';
    ...
    if (!formData.benefits || formData.benefits.length === 0) newErrors.benefits = 'Debes seleccionar al menos un beneficio';
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setTouched({ name: true, description: true, monthlyPrice: true, annualPrice: true });
    if (!validate()) return;
    const planData = {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-xl">
        ...
        <div className="flex-none border-b px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-conexia-green">Editar plan</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5">
          <div className="space-y-5">
            {/* Nombre */}
            <div>
              <label className="block text-sm font-medium text-conexia-green mb-1">Nombre del plan *</label>
              <InputField
                type="text"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-conexia-green mb-1">Precio mensual (ARS) *</label>
                <InputField
                  type="number"
                  placeholder="0.00"
              ...
              <div>
                <label className="block text-sm font-medium text-conexia-green mb-1">Precio anual (ARS) *</label>
                <InputField
                  type="number"
                  placeholder="0.00"
            ...
            {/* Beneficios */}
            <div>
              <h3 className="text-sm font-semibold text-conexia-green mb-2">Beneficios del plan *</h3>
              {errors.benefits && <p className="text-sm text-red-600 mb-2">{errors.benefits}</p>}
              {loadingBenefits ? (
                ...
                <div className="space-y-3">
                  {benefits.map((benefit) => {
                    ...
                    return (
                      <div key={benefit.id} className={`border rounded-lg px-4 py-3 flex flex-col gap-2 ${
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                >
                                  {benefit.options?.values?.map((val) => (
                                    <option key={val} value={val}>{labelify(val)}</option>
                                  ))}
                                </select>
                              </div>
                            )}
