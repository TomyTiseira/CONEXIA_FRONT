...
export default function ServiceReportsDetailGrid({ serviceId }) {
  ...
  return (
    <div className="max-w-6xl mx-auto pt-8 pb-24">
      {/* Header con botón para ver servicio */}
      <div className="bg-white px-6 py-4 rounded-xl shadow-sm relative mb-6 mx-4 md:mx-0">
        <h2 className="text-2xl font-bold text-conexia-green text-center">Reportes del servicio</h2>
        {/* Botón en desktop a la derecha */}
        <div className="hidden sm:block absolute right-6 top-1/2 -translate-y-1/2">
          <Button
            variant="add"
            className="px-4 py-2 text-sm"
            onClick={() => window.location.href = `/services/${serviceId}?from=reports-service&fromReportsServiceId=${serviceId}`}
          >
            Ver servicio
          </Button>
        </div>
        ...
        <table className="min-w-full table-auto text-sm mb-0">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan="5" className="p-6 text-center text-gray-500 italic">No se encontraron reportes para este servicio.</td>
              </tr>
            ) : (
              reports.map(r => (
