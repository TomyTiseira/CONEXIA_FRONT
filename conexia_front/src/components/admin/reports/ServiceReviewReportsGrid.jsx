...
import { fetchServiceReviewReports } from '@/service/reports/fetchServiceReviewReports';
import { LoadingSpinner } from '@/components/ui/LoadingSpinner';

export default function ServiceReviewReportsGrid({ serviceReviewId }) {
  ...
  // Obtener info de la reseña del primer reporte
  const reviewInfo = reports.length > 0 ? reports[0] : null;

  useEffect(() => {
    if (!serviceReviewId) return;
    // No cargar si está en proceso de logout
    if (typeof window !== 'undefined' && window.__CONEXIA_LOGGING_OUT__ === true) {
      return;
    }
    setLoading(true);
    fetchServiceReviewReports(serviceReviewId, page)
      .then(data => {
  ...
  }, [serviceReviewId, page]);

  // Obtener parámetros de retorno desde la URL
  const returnFilter = searchParams?.get('filter') || 'service-reviews';
  const returnOrder = searchParams?.get('order') || 'reportCount';
  const returnPage = searchParams?.get('page') || '1';

  // Obtener serviceId del primer reporte (ahora viene directamente en cada reporte)
  const serviceId = reviewInfo?.serviceId;

  const handleViewServiceReview = () => {
    if (serviceId) {
      router.push(
        `/services/${serviceId}?from=reports-service-review&highlightReviewId=${serviceReviewId}&fromReportsServiceReviewId=${serviceReviewId}`
      );
    } else {
      alert('No se pudo obtener el ID del servicio. Por favor, intenta de nuevo.');
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto pt-8 pb-24">
      <div className="bg-white px-6 py-4 rounded-xl shadow-sm relative mb-6 mx-4 md:mx-0">
        <h2 className="text-2xl font-bold text-conexia-green text-center">
          Reportes de la Reseña de Servicio
        </h2>
        ...
        <div className="hidden sm:block absolute right-6 top-1/2 -translate-y-1/2">
          <Button
            ...
            onClick={handleViewServiceReview}
            disabled={!serviceId}
          >
      ...
      <div className="bg-white rounded-xl shadow-sm border p-0 overflow-x-auto mx-4 md:mx-0">
        <table className="min-w-full table-auto text-sm mb-0">
          <colgroup>
            <col className="w-[60px] md:w-[60px]" />
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan="5" className="p-6 text-center text-gray-500 italic">
                  No se encontraron reportes para esta reseña de servicio.
                </td>
              </tr>
            ) : (
              reports.map(r => (
                <tr key={r.id} className="border-b hover:bg-gray-50 h-auto align-top">
                  ...
                  <td className="p-4 align-top break-words max-w-[160px] text-left">
                    {r.reason}
                    {r.reason === "Otro" && r.otherReason ? ` (${r.otherReason})` : ""}
                  </td>
