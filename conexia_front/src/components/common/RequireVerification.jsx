...
export default function RequireVerification({ children, onUnverified, action = 'realizar esta acción', noBackdrop = false }) {
  ...
  const modalContent = (
    <div className={`fixed inset-0 ${noBackdrop ? '' : 'bg-black bg-opacity-50'} flex items-center justify-center z-50 p-4`}>
      <div className="bg-white rounded-lg max-w-md w-full p-6 shadow-xl">
        <div className="flex items-start space-x-4 mb-6">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-bold text-gray-900 mb-2">Verificación Requerida</h3>
            <p className="text-gray-600">Para {action} debes verificar tu identidad primero.</p>
          </div>
          <button
  ...
  return (
    <>
      <div onClickCapture={handleClick} className={!isVerified ? 'cursor-not-allowed inline-block' : 'inline-block'}>
        {children}
      </div>

      {showWarning && (typeof document !== 'undefined' ? createPortal(modalContent, document.body) : modalContent)}
    </>
  );
}
