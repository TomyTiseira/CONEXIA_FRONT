...
export default function ConnectionRequestCard({ name, profession, image, requestId, senderId, onIgnore, onAccepted }) {
  ...
  <Button variant="cancel" className="px-4 py-1.5 text-sm" onClick={onIgnore}>Rechazar</Button>
      </div>
  ...
  <Button variant="cancel" className="flex-1 px-4 py-1.5 text-sm" onClick={onIgnore}>Rechazar</Button>
      </div>
    </div>
  );
}
