...
export default function NavbarCommunity() {
  ...
  const totalUnread = useMemo(() => {
    try {
      return (Array.isArray(chats) ? chats : []).reduce(
  ...
  const navItems = [
    ...
    { label: "Proyectos", href: "/project/search", icon: Layers },
    {
      ...
      href: "/connections",
      icon: HiUserGroup,
      showDot: connectionRequestsCount > 0,
    },
          ...
          <div className="flex items-center gap-4 text-conexia-green min-w-0 flex-1 justify-end order-3">
            ...
            <div className="relative">
              <Link href="/messaging" prefetch={false} className="block">
                <MessageCircle
                  size={20}
                  className="hover:text-conexia-green/80"
                />
              </Link>
              {displayUnread > 0 && (
                <span
                  className={`absolute -top-1 -right-2 min-w-[20px] h-[16px] px-1 rounded-full bg-[#e6424b] text-white leading-[16px] text-center ${desktopBadgeText}`}
                >
                  {displayUnread}
                </span>
              )}
          ...
          <div className="flex items-center justify-between px-4 py-2 h-[56px]">
            ...
            <div className="flex items-center gap-3 text-conexia-green flex-shrink-0">
              <div className="relative">
                ...
                {displayUnread > 0 && (
                  <span
                    className={`absolute -top-1 -right-2 min-w-[18px] h-[14px] px-1 rounded-full bg-[#e6424b] text-white leading-[14px] text-center ${mobileBadgeText}`}
                  >
                    {displayUnread}
                  </span>
                )}
