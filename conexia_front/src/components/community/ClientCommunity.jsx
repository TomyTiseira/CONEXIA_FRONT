...
export default function ClientCommunity() {
  ...
  useEffect(() => {
    const fetchPublications = async () => {
      ...
      try {
        ...
        const data = Array.isArray(res.data?.publications)
        ...
        setPublications((prev) =>
          page === 1 ? validPublications : [...prev, ...validPublications],
        );
  ...
  const handlePublish = (result) => {
    ...
    if (result.success) {
      ...
      setToast({
        type: "error",
        message: result.error || "No se pudo crear la publicación",
        isVisible: true,
      });
    }
  };
  ...
  const avatar =
    !isInternal && profile?.profilePicture
      ? buildMediaUrl(profile.profilePicture)
      : "/images/default-avatar.png";
  const displayName =
    !isInternal && profile?.name && profile?.lastName
    ...
    <main className="md:p-8 bg-[#f8fcfc] min-h-screen pb-24 md:pb-8 relative">
      <div className="w-full md:max-w-6xl mx-auto flex flex-col md:flex-row gap-0 md:gap-4">
        ...
        <div className="flex-1 flex flex-col items-center md:max-w-2xl md:mx-auto px-3 md:px-0">
          ...
          <div
            className={`bg-white rounded-2xl shadow border border-[#c6e3e4] px-2 sm:px-4 md:px-6 pt-4 pb-2 mb-3 flex flex-col gap-2 w-full ${
          ...
          >
            ...
            <div className="flex items-center gap-2 md:gap-3 mt-0 pb-2 pl-[56px] md:pl-[56px]">
              ...
              >
                ...
                <span className="hidden md:inline text-xs text-conexia-green/80 select-none">
                  Imagen
                </span>
              </button>
              <button
              ...
              <Button
                onClick={() => setModalOpen(true)}
                className="!px-4 md:!px-5 !py-2 !rounded-lg ml-0 md:ml-4 mt-2 md:mt-0"
                disabled={isLoading || !user || !canCreateContent}
              >
                Publicar
              </Button>
          ...
          <div className="flex flex-col gap-0 w-full">
            {!loadingPublications &&
              ...
              !errorPublications && (
                <div className="text-conexia-green/70">
                  No hay publicaciones de la comunidad para mostrar.
                </div>
              )}
            ...
            {loadingPublications && publications.length > 0 && (
              <div className="text-conexia-green/70 py-4 text-center">
                Cargando más publicaciones...
              </div>
            )}
            {!hasMore && publications.length > 0 && (
              <div className="text-conexia-green/60 py-4 text-center">
