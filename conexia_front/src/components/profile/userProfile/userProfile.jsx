...
export default function UserProfile() {
  ...
  const { isVerified: isUserVerified, loading: verificationLoading } =
  ...
  if (storeUser) {
    ...
    isAdmin =
      normalizedRoleName === ROLES.ADMIN ||
      normalizedRole === ROLES.ADMIN ||
      storeUser.roleId === 1;
    isModerator =
      normalizedRoleName === ROLES.MODERATOR ||
  ...
  if (!isAdmin && !isModerator && authUser) {
    ...
    isModerator =
      normalizedAuthRole === ROLES.MODERATOR || authUser.roleId === 3;
  }
    ...
    const refreshProfileOnVerification = async () => {
      if (isOwner && isUserVerified && profile && !profile.profile?.verified) {
        try {
          ...
          console.error(
            "Error al refrescar perfil después de verificación:",
            err,
          );
        }
      }
        ...
        sessionStorage.setItem(
          ...
          JSON.stringify({
            ...
            message: response.success
              ...
              : "El perfil se guardó con advertencias.",
            isVisible: true,
            userId: id,
          }),
        );
  ...
  if (showMyProjects) {
    const MyProjects =
      require("@/components/project/search/MyProjectsView").default;
    return (
      <div className="bg-conexia-soft min-h-screen">
        <Navbar />
        <div className="max-w-5xl mx-auto bg-white rounded-lg shadow p-6 mt-4 mx-6 md:mx-auto">
          ...
          <div className="mt-6 flex justify-end">
            <Button variant="primary" onClick={() => setShowMyProjects(false)}>
              Volver a mi perfil
            </Button>
          </div>
        </div>
  ...
  return (
    <div className="bg-conexia-soft min-h-screen">
        ...
        <div className="bg-white rounded-xl shadow p-6 border border-[#e0e0e0]">
          ...
          <div className="relative h-48 rounded overflow-hidden bg-gray-100 mb-8">
            <Image
              src={
                user.coverPicture
                  ? buildMediaUrl(user.coverPicture)
                  : "/bg-smoke.png"
              }
              alt="Foto de portada"
              layout="fill"
          ...
          <div
            className="relative flex flex-col sm:flex-row sm:items-center mb-4 sm:justify-between"
            style={{ minHeight: 64 }}
          >
          ...
          {!isOwner &&
            !isAdmin &&
            !isModerator &&
            profile.profile?.connectionData?.state === "pending" &&
            profile.profile?.connectionData?.senderId !== storeUser?.id &&
            !accepting && (
                ...
                <div className="flex items-center gap-2 mb-2 sm:mb-0 justify-center sm:justify-start text-center sm:text-left">
                  ...
                  <span className="text-conexia-green text-sm font-medium">
                    {(() => {
                      ...
                      const lastName =
                        profile.profile?.lastName?.split(" ")[0] || "";
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={async () => {
                        setAccepting(true);
                        await acceptRequest(
                          profile.profile?.connectionData?.id,
                        );
          ...
          <div className="mt-6 space-y-6">
            {user.description && (
              <Section title="Descripción">
                <p className="whitespace-pre-line break-words overflow-x-auto max-w-full">
                  {user.description}
                </p>
              </Section>
            )}
                  ...
                  {user.experience.map((exp, idx) => {
                    ...
                    return (
                      <li key={idx} className="mb-2">
                        ...
                        {exp.project && (
                          <div className="text-sm text-conexia-coral">
                            {exp.project}
                          </div>
                        )}
                ...
                <ul className="ml-2">
                  {user.education.map((edu, idx) => {
                    const start = edu.startDate
                      ? new Date(edu.startDate).toLocaleDateString("es-AR", {
                          year: "numeric",
                          month: "short",
                        })
                      : "";
                    const end = edu.isCurrent
                      ? "Actualidad"
                      : edu.endDate
                        ? new Date(edu.endDate).toLocaleDateString("es-AR", {
                            year: "numeric",
                            month: "short",
                          })
                        : "";
                    ...
                    {user.certifications.map((cert, idx) => (
                      <li key={idx}>
                        ...
                        <a
                          href={cert.url}
                          target="_blank"
                          className="text-conexia-coral underline"
                        >
                          Ver certificación
                        </a>
                      </li>
                  ...
                  <ul className="list-disc ml-6">
                    {user.socialLinks.map((link, idx) => (
                      <li key={idx} className="break-words max-w-full">
                        <span className="font-semibold mr-2 align-top">
                          {link.platform}:
                        </span>
                        <a
                          href={link.url}
      ...
      {!(isAdmin || isModerator) && <MessagingWidget avatar={avatar} />}
    </div>
  );
}
