...
export default function EditProfileForm({
  ...
  useEffect(() => {
    ...
    if (form.socialLinks.length > 0) {
      setSocialErrors(
        form.socialLinks.map((social) => getSocialErrors(social)),
      );
      setSocialTouched(
        form.socialLinks.map(() => ({ platform: false, url: false })),
      );
    }
    ...
    if (form.certifications.length > 0) {
      setCertErrors(
        form.certifications.map((cert) => getCertificationErrors(cert)),
      );
      setCertTouched(
        form.certifications.map(() => ({ name: false, url: false })),
      );
    }
  }, []);
    ...
    switch (name) {
      ...
      case "birthDate":
        if (value) {
          ...
          if (isCurrentOrFutureDate(value))
  ...
  const getExperienceErrors = (exp) => {
    const errors = {
      ...
      project:
        !exp.project || exp.project.trim() === "" ? "Campo obligatorio" : "",
  ...
  const getCertificationErrors = (cert) => {
    const errors = {
      ...
      url:
        !cert.url || cert.url.trim() === ""
          ? "Campo obligatorio"
          : cert.url && cert.url.trim() !== "" && !isValidURL(cert.url.trim())
            ? "Ingresá una URL válida"
            : "",
    };
    ...
    if (isCertificationValid(cert) && !cert.confirmed) {
      errors.confirmation =
        "Debes confirmar la certificación antes de continuar";
    }
  ...
  function isCertificationValid(cert) {
    return (
      cert.name && cert.name.trim() !== "" && cert.url && cert.url.trim() !== ""
    );
  }
  ...
  function handleAddExperience() {
    ...
    setExpTouched((prev) => [
      ...prev,
      { title: false, project: false, startDate: false, endDate: false },
    ]);
    setExpErrors((prev) => [
      ...prev,
      getExperienceErrors({
  ...
  function handleExpBlur(i, field) {
    setExpTouched((prev) =>
      prev.map((t, idx) => (idx === i ? { ...t, [field]: true } : t)),
    );
  ...
  function handleAddSocial() {
    setForm((prev) => ({
      ...
      socialLinks: [
        ...prev.socialLinks,
        { platform: "", url: "", confirmed: false },
      ],
    }));
    setSocialTouched((prev) => [...prev, { platform: false, url: false }]);
    setSocialErrors((prev) => [
      ...prev,
      getSocialErrors({ platform: "", url: "" }),
    ]);
  }
  ...
  function handleAddEducation() {
    ...
    setEduTouched((prev) => [
      ...prev,
      { institution: false, title: false, startDate: false, endDate: false },
    ]);
    setEduErrors((prev) => [
      ...prev,
      getEducationErrors({
  ...
  function handleAddCertification() {
    setForm((prev) => ({
      ...
      certifications: [
        ...prev.certifications,
        { name: "", url: "", confirmed: false },
      ],
    }));
    setCertTouched((prev) => [...prev, { name: false, url: false }]);
    setCertErrors((prev) => [
      ...prev,
      getCertificationErrors({ name: "", url: "" }),
    ]);
  }
  ...
  function handleImageChange(e, field) {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        setImgErrors((prev) => ({
          ...prev,
          [field]: "La imagen no puede superar los 5MB",
        }));
  ...
  function handleSubmit(e) {
    ...
    const newSocialTouched = form.socialLinks.map(() => ({
      platform: true,
      url: true,
      confirmation: true,
    }));
    ...
    const newEduTouched = form.education.map(() => ({
      institution: true,
      title: true,
      startDate: true,
      endDate: true,
    ...
    const newCertTouched = form.certifications.map(() => ({
      name: true,
      url: true,
      confirmation: true,
    }));
    ...
    if (
      ...
      setTimeout(() => {
        if (firstErrorField) {
          ...
          const expElement = Array.from(expElements).find(
            (el) =>
              el.getAttribute("data-experience-index") ===
              firstExpErrorIndex.toString(),
          );
          if (expElement) {
          ...
          const socialElement = Array.from(socialElements).find(
            (el) =>
              el.getAttribute("data-social-index") ===
              firstSocialErrorIndex.toString(),
          );
          if (socialElement) {
          ...
          const eduElement = Array.from(eduElements).find(
            (el) =>
              el.getAttribute("data-education-index") ===
              firstEduErrorIndex.toString(),
          );
          if (eduElement) {
          ...
          const certElement = Array.from(certElements).find(
            (el) =>
              el.getAttribute("data-certification-index") ===
              firstCertErrorIndex.toString(),
          );
          if (certElement) {
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-6 p-6">
          ...
          <div className="text-center pb-6 border-b border-gray-200">
            ...
            <p className="text-conexia-green-dark mt-2 text-base md:text-lg">
              Actualiza tu información para mantener tu perfil al día y mejorar
              tus oportunidades de conexión.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div
                onClick={() =>
                  !form.profilePicture && profilePicRef.current?.click()
                }
                className={`
            ...
            <div>
              ...
              <div
                onClick={() =>
                  !form.coverPicture && coverPicRef.current?.click()
                }
                className={`
                  min-h-[280px] flex items-center justify-center
                  border-2 border-dashed rounded-lg p-4 text-center transition-colors
          ...
          {/* Habilidades */}
          <div className="-mt-6">
            <label className="block font-medium text-conexia-green mb-1">
          ...
          <div>
            ...
            {form.experience.map((exp, i) => (
              <div
                key={i}
                className="grid gap-2 mt-2 border-b pb-4"
              ...
              >
                <div className="grid md:grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <InputField
                      value={exp.project}
                      onChange={(e) =>
                        handleExpChange(i, "project", e.target.value)
                      }
                ...
                <div className="grid md:grid-cols-3 gap-2 items-end">
                  <div className="flex flex-col min-h-[80px]">
                    <label className="text-sm text-conexia-green mb-1">
                      Desde
                    </label>
                    <input
                      type="date"
                      ...
                      min={
                        form.birthDate ? getNextDay(form.birthDate) : undefined
                      }
                    ...
                    <div style={{ minHeight: 20 }}>
                      {expTouched[i]?.startDate && expErrors[i]?.startDate && (
                        <p className="text-xs text-red-600 mt-1 text-left">
                          {expErrors[i].startDate}
                        </p>
                      )}
                    </div>
                  </div>
                  <div className="flex flex-col min-h-[80px]">
                    ...
                    />
                    <div style={{ minHeight: 20 }}>
                      {expTouched[i]?.endDate && expErrors[i]?.endDate && (
                        <p className="text-xs text-red-600 mt-1 text-left">
                          {expErrors[i].endDate}
                        </p>
                      )}
                ...
                {!exp.confirmed &&
                  ...
                  isExperienceValid(exp) && (
                    <p className="text-xs text-red-600 mt-1 mb-4">
                      Para continuar debes confirmar la experiencia
                    </p>
                  )}
                <div className="flex justify-end gap-2 items-end">
                  ...
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className={`h-4 w-4 ${exp.confirmed ? "text-green-600" : "text-white"}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
              ...
              >
                <div className="grid md:grid-cols-2 gap-2">
                  <div>
                    ...
                    <InputField
                      ...
                      error={
                        eduTouched[i]?.institution && eduErrors[i]?.institution
                      }
                ...
                <div className="grid md:grid-cols-3 gap-2 items-end">
                  <div className="flex flex-col min-h-[80px]">
                    ...
                    />
                    <div style={{ minHeight: 20 }}>
                      {eduTouched[i]?.startDate && eduErrors[i]?.startDate && (
                        <p className="text-xs text-red-600 mt-1 text-left">
                          {eduErrors[i].startDate}
                        </p>
                      )}
                    </div>
                  </div>
                  <div className="flex flex-col min-h-[80px]">
                    ...
                    />
                    <div style={{ minHeight: 20 }}>
                      {eduTouched[i]?.endDate && eduErrors[i]?.endDate && (
                        <p className="text-xs text-red-600 mt-1 text-left">
                          {eduErrors[i].endDate}
                        </p>
                      )}
                ...
                {!edu.confirmed && eduConfirmError && isEducationValid(edu) && (
                  <p className="text-xs text-red-600 mt-1 mb-4">
                    Para continuar debes confirmar la educación
                  </p>
                )}
                <div className="flex justify-end gap-2 items-end">
                  <button
                  ...
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className={`h-4 w-4 ${edu.confirmed ? "text-green-600" : "text-white"}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
              ...
              <div key={i} className="mt-2" data-certification-index={i}>
                <div className="grid md:grid-cols-3 gap-2 items-center">
                  <div className="flex flex-col items-start h-full">
                    <label className="block font-semibold text-conexia-green text-sm mb-0">
                      Nombre
                    </label>
                    <InputField
                      value={cert.name}
                      onChange={(e) =>
                        handleCertificationChange(i, "name", e.target.value)
                      }
                  ...
                  <div className="flex flex-col items-start h-full">
                    ...
                    <InputField
                      value={cert.url}
                      onChange={(e) =>
                        handleCertificationChange(i, "url", e.target.value)
                      }
                  ...
                  <div className="flex flex-row justify-end items-center gap-2 h-full">
                    ...
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className={`h-4 w-4 ${cert.confirmed ? "text-green-600" : "text-white"}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                ...
                {!cert.confirmed &&
                  ...
                  isCertificationValid(cert) && (
                    <p className="text-xs text-red-600 mt-0">
                      Para continuar debes confirmar la certificación
                    </p>
                  )}
              </div>
            ))}
              ...
              <div key={i} className="mt-2" data-social-index={i}>
                <div className="grid md:grid-cols-3 gap-2 items-center">
                  <div className="flex flex-col items-start h-full">
                    ...
                    {socialTouched[i]?.platform &&
                      socialErrors[i]?.platform && (
                        <p className="text-xs text-red-600 mt-1 text-left">
                          {socialErrors[i].platform}
                        </p>
                      )}
                  ...
                  <div className="flex flex-row justify-end items-center gap-2 h-full">
                    ...
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className={`h-4 w-4 ${link.confirmed ? "text-green-600" : "text-white"}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                ...
                {!link.confirmed &&
                  socialConfirmError &&
                  isSocialLinkValid(link) && (
                    <p className="text-xs text-red-600 mt-0">
                      Para continuar debes confirmar la red social
                    </p>
                  )}
              </div>
            ))}
      ...
      <div
        className="bg-conexia-soft w-full pb-16 md:pb-4"
        style={{ height: 20 }}
      ></div>
    </div>
  );
