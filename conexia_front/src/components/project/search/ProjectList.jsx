...
export default function ProjectList({
  ...
  return (
    <div className={`${reserveGridSpace ? "min-h-[800px] flex flex-col" : ""}`}>
      <div
        className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ${reserveGridSpace ? "flex-1" : ""}`}
      >
        {filteredProjects.map((project) => {
          ...
          return (
            ...
            >
              ...
              <div
                className="flex flex-row gap-2 mb-0.5 w-full justify-between items-center px-1.5"
                style={{
                  minHeight:
                    !project.isActive || project.deletedAt || projectFinished
                      ? "20px"
                      : "8px",
                }}
              >
                {(!project.isActive || project.deletedAt) && (
                  <span
                    className="bg-gray-200 text-gray-700 px-2 py-0.5 rounded text-[12px] font-semibold border border-gray-300 shadow select-none whitespace-nowrap"
                ...
                {projectFinished && (
                  <span
                    className="bg-red-100 text-red-700 px-2 py-0.5 rounded text-[12px] font-semibold border border-red-200 shadow select-none whitespace-nowrap"
                    style={{ lineHeight: "1.1" }}
                  >
                    ⏰ Finalizado
                  </span>
        ...
        {reserveGridSpace &&
          ...
          Array.from({ length: 12 - filteredProjects.length }).map(
            (_, index) => (
